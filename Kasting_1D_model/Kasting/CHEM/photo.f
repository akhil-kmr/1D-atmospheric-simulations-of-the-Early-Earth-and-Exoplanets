...
c-AJ 12/21/2022 ADD new correlated k table
...
c  09/08/2023 AJ Add quantum yield for far UV loop
      data FRO2/1.,1.,1.,1.,1.,1.,1.,0.22,0.61,0.56/

c     add the write file for debug     
      DIRIO = 'IO'
      OPEN(unit=100,file= DIRIO//'/Debug_corrk.dat')
      OPEN(unit=101,file= DIRIO//'/Debug_corrk_trackPHOTON.dat')
C
      PM = 1.67E-24
      BK = 1.38E-16
      RGAS = 8.3143E7
...
C ***** SET PHOTOLYSIS RATES TO ZERO INITIALLY *****
      DO 1 I=1,NZ
      PO2(I) = 0.
      PO3(I) = 0.
      PO3D(I) = 0.
...
C     Note: These column depths are only used for band model calculations.
C     They have no effect when mulu
      HA = RMG*T(NZ)
      HAD = HA*DEN(NZ)
...
C-JK  Note: The A&F band model is no longer used. We continue to read in
C     the coefficients from the datafile, 'photos.pdat', though, to
C     make it easy for others to use this code. The coefficients for
C     nitric oxide predissociation are still used, however, so don't
C     delete this entire section without replacing them.
...
C     Note: The first 17 bands (out of 35) contain the O2 Schumann-
C     Runge bands. This part has been updated to the new correlated-k
C     parameterization. Commented out lines refer to the old exponential
C     sum parameterization. These have been left in, so that one can
C     see where the new coefficients have replaced the old ones.

      DO 19 L=1,35
      WRITE(100,*)
      WRITE(100,*)'L=',L
      WRITE(100,*)
      IF(L.NE.1 .AND. L.NE.10 .AND. L.NE.17) GO TO 1001
      WRITE(101,*)
      WRITE(101,*)'L = ',L
 1001 CONTINUE
      IF (L.LE.17) GO TO 27
...
C
  ...
  28  CONTINUE
      ...
      KN = 1  
      ALP = 1.
c      IF (IO2.EQ.1 .AND. L.LE.17) KN = NK(L)
      IF (IO2.EQ.1 .AND. L.LE.17) KN = KNEW
C
      M=0
...
c      DO 19 K=1,KN
      ...
      IF (IO2.EQ.0) GO TO 20   !This is
      IF (L.GT.17) GO TO 66
C      ALP = ALPHAOLD(L,K) !This is for exponential sum fits
      ALP = ALPHA(K) ! Correlated-k tables with 4 pts

      WRITE(100,401)K,ALP
  400 FORMAT(A,3X,1PE12.5) 
  401 FORMAT('K=',I2,2X,'WEIGHT=',1PE10.3)

      DO 65 I=1,NZ
  65  SO2(I) = SO2HZ(L) + BETA(K,L,I)
  300 FORMAT(I2,1PE12.5)
  301 FORMAT(A10,1PE12.5)
  66  CONTINUE
C
...
C   Note: We rarely use this option anymore, as the two-stream model
C   seems to have no problem handling high absorption optical depths.
...
C
  ...
  21  FLX = FLUX(L)*AGL*ALP
      ...
      DO 23 I=1,NZ
      PO2D(I) = PO2D(I) + FLX*SO2(I)*RO2(L,I)*S(I)
      PO2(I) = PO2(I) + FLX*SO2(I)*(1.-RO2(L,I))*S(I)
...
c     PSO(I) = PSO(I) + FLX*SSO(L)*S(I)  ! SO photolysis zeroed out
...
C   SKIP MULTIPLE SCATTERING ROUTINE IF THE PURE ABSORPTION OPTICAL
C   DEPTH AT 20 KM IS MUCH GREATER THAN THE SCATTERING OPTICAL DEPTH
C   Note: As in the shortwave loop, we rarely exercise this option.
...
C-as Methane has been added as major absorber (03/24/2004)
     ...
     &  +SIGMA(3,L)*TH2O(I)+SSO2A(L)*TSO2(I)+SCH4(L)*TCH4(I))*AM)
      ...
      PO2(I) = PO2(I) + (1.-FRO2(L))*QQ*SIGMA(1,L)*AGL
      PO2D(I) = PO2D(I) + FRO2(L)*QQ*SIGMA(1,L)*AGL
      PCO2D(I) = PCO2D(I) + QQ*SIGMA(2,L)*AGL
      PH2O(I) = PH2O(I) + QQ*SIGMA(3,L)*AGL
      PSO2(I) = PSO2(I) + QQ*SSO2A(L)*AGL
c     PSO(I) = PSO(I) + QQ*SSOA(L)*AGL    ! SO photolysis zeroed out
      PCH4(I) = PCH4(I) + QQ*SCH4(L)*AGL    
  50  CONTINUE
C
C ***** FILL UP RATE MATRIX *****
C   Note: This is where the photorates were loaded into the rate
C   constant matrix in the old code. This is now done in the main
C   code, following the call to subroutine PHOTO.
**********************************

      if(N.eq.1) then
c Notice that for the 10th interval in the far UV SFX is in
c  photons/s/cm^2/A    
      do L1=10,1,-1      
