...
class ReadRate(object):
    ...
    def rev_rate(self, var, atm):
        ...
        for i in rev_list:
            ...
            if np.any(var.k[i] > 1.e-8): print ('R' + str(i) + " " + var.Rf[i-1] +' :  ' + str(np.amax(var.k[i])) )
            if np.any(var.k[i-1] > 1.e-8): print ('R' + str(i-1) + " " + var.Rf[i-1] + ' :  ' + str(np.amax(var.k[i-1])) )        
                            ...
                            if vulcan_cfg.fix_species_from_coldtrap_lev == True:
                                ...
                                if sp == 'H2O_l_s' or sp == 'H2SO4_l' or sp == 'NH3_l_s' or sp == 'S8_l_s': atm.conden_min_lev[sp] = nz-1 # fix condensates through the whole amtosphere 
                                    ...
                                    if list(var.y[conden_status,species.index(sp)]): # if it condenses
                                        ...
                                        conden_min_lev = int( np.where(atm.sat_mix[sp] == min_sat)[0] )
                                        atm.conden_min_lev[sp] = conden_min_lev
                                        #print (atm.conden_min_lev[sp])
...
#         atm.Hpi = Hpi
    ...
    def loss(self, data_var): 
        ...
        for atom in atom_list:
            ...
            # TEST V scaling
            if atom not in vulcan_cfg.loss_ex: # shami added 2024
...
class Ros2(ODESolver):
    ...
    def solver(self, var, atm, para):
        ...
        if vulcan_cfg.use_fix_layer_co2 == True:
            sol[:13,species.index('CO2')] = vulcan_cfg.use_fix_sp_bot['CO2']*atm.n_0[:13]
...
class Output(object):
    ...
    def print_end_msg(self, var, para ): 
        ...
        for atom in vulcan_cfg.atom_list: 
            if atom not in vulcan_cfg.loss_ex:
    ...
    def plot_evo(self, var, atm, plot_j=-1, dn=1, abun_min=1e-12, plot_min_time = 1e6):
        ...
        for i,sp in enumerate(vulcan_cfg.plot_spec):
            plt.plot(var.t_time[::dn], ymix_time[::dn,plot_j,species.index(sp)],label=sp) # ,c = plt.cm.rainbow(float(i)/len(plot_spec))
        ...
        plt.savefig(plot_dir + vulcan_cfg.out_name+ '-evo.png')
        if vulcan_cfg.use_PIL == True:
            plot = Image.open(plot_dir + vulcan_cfg.out_name+ '-evo.png')
            plot.show()
            plt.close()
        else: plt.show(block = False)
    
    def plot_evo_inter(self, var, atm, plot_j=-1, dn=1, abun_min=1e-12):
        '''
